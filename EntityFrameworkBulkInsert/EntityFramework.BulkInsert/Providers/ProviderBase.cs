...
namespace EntityFramework.BulkInsert.Providers
{
        ...
        where TTransaction : IDbTransaction
    {
        ...
        public virtual void Run<T>(IEnumerable<T> entities)
        {
            using (var dbConnection = GetConnection())
            {
                ...
                using (var transaction = dbConnection.BeginTransaction())
                {
                    try
                    {
                        Run(entities, (TConnection)dbConnection, (TTransaction)transaction);
                        transaction.Commit();
                    }
                    catch (Exception)
                    {
        ...
        {
            Run(entities, (TConnection)((TTransaction)transaction).Connection, (TTransaction)transaction);
        }

        public virtual void Run<T>(IEnumerable<T> entities, IDbConnection dbConnection)
        {
            Run(entities, (TConnection)dbConnection, default(TTransaction));
        }

   
        public abstract void Run<T>(IEnumerable<T> entities, TConnection connection, TTransaction transaction);

        public void Run<T>(IEnumerable<T> entities, IDbConnection dbConnection, IDbTransaction transaction)
        {
            Run(entities, (TConnection)dbConnection, (TTransaction)transaction);
        }
    }
